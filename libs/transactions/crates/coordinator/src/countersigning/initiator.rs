...
#[hdk_extern]
pub fn attempt_create_transaction(input: AttemptCreateTransactionInput) -> ExternResult<Record> {
    ...
    let _counterparty_action_hash: ActionHash = result.decode().map_err(|e| {
        wasm_error!(WasmErrorInner::Guest(format!(
            "Failed to decode response of request_create_transaction remote call: {}",
            e
        )))
    })?;
...
fn build_preflight_request(
    ...
    let bytes = SerializedBytes::try_from(transaction.clone()).map_err(|e| {
        wasm_error!(WasmErrorInner::Guest(format!(
